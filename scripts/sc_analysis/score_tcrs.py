...
import pandas as pd
import numpy as np
def process_gene(chain,df_merge):
    temp = df_merge['TRB'+chain+'_1'].str.split('TRB'+chain,expand=True)[1].str.split('-',expand=True)
    temp.fillna(value=1,inplace=True)
    temp[0] = temp[0].astype(int).map("{:02}".format)
    temp[1] = temp[1].astype(int).map("{:02}".format)
    temp[2] = 'TCRB' + chain + temp[0].astype(str) + '-'+temp[1].astype(str)
    return temp[2]
...
df_merge['TRBV_1'] = process_gene('V',df_merge)
df_merge['TRBJ_1'] = process_gene('J',df_merge)
